var trend_canvas_graph = null;
function opTrend_graph() {
    this.defaultTrendServerName = 'trend_getData';
    this.canvasX = 0;
    this.canvasY = 0;
    ...
    this.margin_bottom = 10;// 趋势内部面板与边框下间隔
    ...
    this.widthRatioA = 10;
    this.widthRatioB = 11;//趋势区域所占整体宽度为widthRatioA/widthRatioB，上下限所占（B-A）/B
    ...
    this.isComponents = true;//true 为图形引用
    ...
    this.draw = function () {
        ...
        if (!this.isComponents) {
            trend_canvas_graph.clearRect(
                this.canvasX,
                this.canvasY,
                this.canvasWidth + this.margin_padding * 2,
                this.canvasHeight + this.margin_top + this.margin_bottom);
        }
        // 背景
    ...
    this.drawPrompt = function (str) {
        trend_canvas_graph.fillStyle = "red";
        trend_canvas_graph.font = "15px 宋体";
        trend_canvas_graph.fillText(str, this.canvasX
            + this.canvasWidth / this.widthRatioB
            * this.widthRatioA / 2, this.canvasY
            + this.canvasHeight / 2);
    };
    /**
     * 更新X轴坐标时间
    ...
    this.updateTime = function () {
        this.from = new Date(this.from.getTime()
            + this.interval);
        this.to = new Date(this.to.getTime()
            + this.interval);
    };
    this.init = function () {
        this.from = this.getTimeFromString(this.from);
    ...
    this.trendObj = function () {
        ...
        this.draw = function () {
            ...
            if (this.data != null && this.data.length > 1 && !this.hide) {
                this.opTrend
                    .repaintLine(this.data, y, this.color, this.lineWidth,
                    this.type, this.topLimit, this.lowLimit);
            } else if (this.trendData != null) {
                this.data = this.opTrend.drawLineAndGenerateData(
                    this.trendData, y, this.color, this.lineWidth,
                    this.type, this.topLimit, this.lowLimit, this.hide);
                this.trendData = null;
            }
        };
        //绘制限高限低线
        this.drawAlarmLine = function () {
            var y = this.opTrend.canvasY + this.opTrend.canvasHeight
                + this.opTrend.margin_top;
            var x1 = this.opTrend.canvasX
                + this.opTrend.margin_padding, x2 = this.opTrend.canvasX
                + this.opTrend.canvasWidth
                * this.opTrend.widthRatioA
                / this.opTrend.widthRatioB
                + this.opTrend.margin_padding;
            var height = 16, tempH = 4, tempW = 2;
            if (this.alarmTop != null && this.alarmTop != ""
                && !this.hide) {
                var width = trend_canvas_graph
                        .measureText(this.alarmTop).width * 1.2;

                var yy = this.opTrend.getCanvasY(this.alarmTop,
                    this.topLimit, this.lowLimit);
                yy = this.opTrend.drawLineLimit(yy, y);
                trend_funStyle_graph.drawDashedLine(x1 + width, yy, x2
                    - width, yy, 2, this.lineWidth, this.color);

                trend_canvas_graph.globalAlpha = 0.6;
                trend_canvas_graph.fillStyle = this.color;
                trend_canvas_graph.fillRect(x1, yy - height / 2, width,
                    height);
                trend_canvas_graph.fillRect(x2 - width,
                    yy - height / 2, width, height);
                trend_canvas_graph.globalAlpha = 1;
                trend_canvas_graph.fillStyle = this.color;
                trend_canvas_graph.font = "12px 宋体";
                trend_canvas_graph.fillText(this.alarmTop, x1 + tempW,
                    yy + tempH);
                trend_canvas_graph.fillText(this.alarmTop, x2 - width
                    + tempW, yy + tempH);
            }
            if (this.alarmLow != null && this.alarmLow != ""
                && !this.hide) {
                var width = trend_canvas_graph
                        .measureText(this.alarmLow).width * 1.2;
                ...
                trend_canvas_graph.fillStyle = this.color;
                trend_canvas_graph.font = "12px 宋体";
                trend_canvas_graph.fillText(this.alarmLow, x1 + tempW,
                    yy + tempH);
                trend_canvas_graph.fillText(this.alarmLow, x2 - width
                    + tempW, yy + tempH);
            }
        },
            // 绘制右边上下限图形及文字
            this.drawRightMessage = function (temp) {
                ...
                y = this.opTrend.canvasY + this.opTrend.margin_top,
                    width = this.opTrend.canvasWidth
                        / this.opTrend.widthRatioB,
                    height = this.opTrend.canvasHeight,
                    side = height / 35, x1 = x + width
                    / this.opTrend.widthRatioB, y1 = y + temp,
                    y2 = y + height - temp;
                trend_canvas_graph.fillStyle = this.color;
                trend_canvas_graph.fillRect(x1, y1, side, side);
                trend_canvas_graph.fillRect(x1, y2, side, side);
                trend_canvas_graph.font = this.opTrend.timeFontSize
                    + "px Verdana";
                var t = Math.round(this.topLimit), l = Math
                    .round(this.lowLimit);
                trend_canvas_graph.fillText(t, x1 + side * 1.5, y1 + side);
                trend_canvas_graph.fillText(l, x1 + side * 1.5, y2 + side);
            ...
            this.update = function () {
                ...
                //					var timeWidth = null;
                //					timeWidth = (this.opTrend.to.getTime() - this.opTrend.from.getTime()) / 1000;
                //					var rse = (this.opTrend.canvasWidth * (this.opTrend.widthRatioA/ this.opTrend.widthRatioB)) / timeWidth;
                //					for ( var j = 0; j < this.data.length; j++) {
                //						this.data[j][0] -= rse * this.opTrend.interval / 1000;
                //					}
                //				}
    ...
    this.borderRect = {
        draw: function (opTrend) {
            ...
            // x,y,width,height,flag,lineWidth,color,fColor,bcolor
            trend_funStyle_graph.drawRect(dashedRect);
        }
    };
    /**
     * 背景线
    ...
    this.trendBackLine = {
        draw: function (opTrend) {
            ...
            for (var i = 0; i < opTrend.trend_backLineCount - 1; i++) {
                trend_funStyle_graph.drawDashedLine(x, temp + y,
                    opTrend.canvasWidth + x
                    - opTrend.canvasWidth
                    / opTrend.widthRatioB, temp + y,
    ...
    this.trendRect = {
        draw: function (opTrend) {
            ...
            // x,y,width,height,lineStyle,lineWidth,color,fColor,bcolor
            trend_funStyle_graph.drawRect(dashedRect);
        }
    };
    /**
    ...
    this.canvasXAxis = {
        ...
        scale: {
            ...
            draw: function (opTrend) {
                ...
                trend_canvas_graph.fillStyle = opTrend.timeFontColor;
                for (var i = 0; i < opTrend.xAxisScaleCount + 1; i++) {
                    ...
                    ;
                    trend_canvas_graph.font = opTrend.timeFontSize
                        + "px Verdana";
                    ...
                    ;
                    trend_canvas_graph.fillText(hms, _x, y + this.lineHeight
                        * 2);
                    trend_canvas_graph.fillText(ymd, _x, y + this.lineHeight
                        * 4);
                    xTemp += temp;
                }
                ;
                // objArea.canvasXAxis.update();
                trend_canvas_graph.stroke();
            }
        },
        // 绘制背景框
        draw: function (opTrend) {
    ...
    this.getXProportion = function () {
        var timeWidth = this.to.getTime()
            - this.from.getTime();
        return (this.canvasWidth - this.canvasWidth
            / this.widthRatioB)
            / timeWidth;
    };
    /**
    ...
    this.drawLineAndGenerateData = function (data, y, color, lineWidth, type, top,
        ...
        if (!isHide) {
            this.repaintLine(newData, y, color, lineWidth, type, top,
                low);
        }
        return newData;
    };
    ...
    this.repaintLine = function (data, y, color, lineWidth, type, top, low) {
        ...
        trend_canvas_graph.beginPath();
        trend_canvas_graph.lineWidth = lineWidth;
        trend_canvas_graph.strokeStyle = color;
        // var tempX = this.canvasX,tempY = this.canvasY;
        if (data.length > 0) {

            for (var i = 0; i < data.length; i++) {
                var xx = this.getCanvasX(data[i][0])
                    + this.canvasX + this.margin_padding, yy = data[i][1], rightX = this.canvasX
                    + this.canvasWidth
                    * this.widthRatioA
                ...
                if (yy == null && i + 1 < data.length) {
                    ...
                    trend_canvas_graph.moveTo(x1, y1);
                } else {
                    if (yy != null) {
                        ...
                        //低版本IE需先移动点至曲线第一个点
                        if (i == 0) {
                            trend_canvas_graph.moveTo(xx, yy);
                        }
                        if (this.isComponents) {
                            if (xx >= this.canvasX
                                + this.margin_padding) {

                                trend_canvas_graph.lineTo(xx, yy);
                            } else {
                                trend_canvas_graph.moveTo(this.canvasX
                                    + this.margin_padding, yy);
                            }
                        } else {
                            trend_canvas_graph.lineTo(xx, yy);
                        }
...
var trend_funStyle_graph = {
    ...
    drawRect: function (obj) {
        ...
        switch (obj.fillType) {
            case 0:
                trend_canvas_graph.fillStyle = "white";
                trend_funStyle_graph.drawBorderRect(obj);
                break;
            case 1:
                if (obj.fcolor == null || obj.fcolor == undefined)
                    obj.fcolor = "white";
                trend_canvas_graph.fillStyle = obj.bcolor;
                trend_funStyle_graph.drawBorderRect(obj);
                break;
            case 2:
                trend_canvas_graph.fillStyle = obj.bcolor;
                trend_funStyle_graph.drawBorderRect(obj);
                trend_funStyle_graph.drawBgStyle(obj);
                break;
            case 3:
                grd = trend_funStyle_graph.drawBgColorChange(obj);
                trend_canvas_graph.fillStyle = grd;
                trend_funStyle_graph.drawBorderRect(obj);
                break;
        }
        ;
    ...
    drawBgColorChange: function (obj) {
        ...
        switch (obj.secondParam) {
            case 0:
                grd = trend_canvas_graph.createLinearGradient(0, obj.y, 0,
                    obj.height + obj.y);
                break;
            case 1:
                grd = trend_canvas_graph.createLinearGradient(obj.x, 0, obj.width
                    + obj.x, 0);
                break;
            case 2:
                grd = trend_canvas_graph.createLinearGradient(obj.x, obj.y,
                    obj.width + obj.x, obj.height + obj.y);
                break;
            case 3:
                grd = trend_canvas_graph.createLinearGradient(obj.x, obj.height
                    + obj.y, obj.width + obj.x, obj.y);
                break;
        }
    ...
    drawBgStyle: function (obj) {
        ...
        switch (obj.firstParam) {
            case 0:// 横线
                trend_funStyle_graph.drawLine4Lateral(a, obj);
                break;
            case 1:// 纵线
                trend_funStyle_graph.drawLine4Longitudinal(a, obj);
                break;
            case 2:// 横纵交叉线
                trend_funStyle_graph.drawLine4Lateral(a, obj);
                trend_funStyle_graph.drawLine4Longitudinal(a, obj);
                break;
            case 3:// 横线
                trend_funStyle_graph.drawLine4Lateral(a / 2, obj);
                break;
            case 4:// 纵线
                trend_funStyle_graph.drawLine4Longitudinal(a / 2, obj);
                break;
            case 5:// 纵横交叉线
                trend_funStyle_graph.drawLine4Lateral(a / 2, obj);
                trend_funStyle_graph.drawLine4Longitudinal(a / 2, obj);
                break;
            case 6:// k = 1
                trend_funStyle_graph.drawLine4beveled(b, obj);
                break;
            case 7:// k = -1
                trend_funStyle_graph._drawLine4beveled(b, obj);
                break;
            case 8:// 交叉线
                trend_funStyle_graph.drawLine4beveled(b, obj);
                trend_funStyle_graph._drawLine4beveled(b, obj);
                break;
            case 9:// k = 1
                trend_funStyle_graph.drawLine4beveled(b / 2, obj);
                break;
            case 10:// k = -1
                trend_funStyle_graph._drawLine4beveled(b / 2, obj);
                break;
            case 11:// 交叉线
                trend_funStyle_graph.drawLine4beveled(b / 2, obj);
                trend_funStyle_graph._drawLine4beveled(b / 2, obj);
                break;
            case 12:// 波浪线
                trend_funStyle_graph.drawWavesLine(obj);
                break;
            case 13:// 砖块线
                trend_funStyle_graph.drawBrickBG(obj);
                break;
            case 14:// 横线虚线
                trend_funStyle_graph.drawDashedLine4BGLateral(a, obj);
                break;
            case 15:// 纵向虚线
                trend_funStyle_graph.drawDashedLine4BGLongitudinal(a, obj);
                break;
        }
    },
    ...
    drawLine4Longitudinal: function (temp, obj) {
        ...
        while ((obj.x + count) < obj.width + obj.x) {
            trend_funStyle_graph.drawDashedLine(obj.x + count, obj.y, obj.x
                + count, obj.y + obj.height, 0, 1, obj.fcolor);
            count += temp;
        }
        ;
    },
    ...
    drawLine4Lateral: function (temp, obj) {
        ...
        while ((obj.y + count) < obj.height + obj.y) {
            trend_funStyle_graph.drawDashedLine(obj.x, obj.y + count, obj.x
    ...
    drawLine4beveled: function (temp, obj) {
        ...
        while (obj.x + flag + count < obj.x + obj.width) {
            trend_funStyle_graph
                .drawDashedLine(obj.x + count, obj.y + obj.height, obj.x
                + flag + count, obj.y, 0, 1, obj.fcolor);
            count += temp;
        }
        ;
        ...
        while (obj.y + obj.height - count > obj.y) {
            trend_funStyle_graph.drawDashedLine(obj.x + obj.width - count,
                obj.y + obj.height, obj.x + obj.width, obj.y + obj.height
                - count, 0, 1, obj.fcolor);
            count += temp;
        }
        ;
    },
    ...
    _drawLine4beveled: function (temp, obj) {
        ...
        var flag = 0;
        while ((obj.y + count) <= obj.height + obj.y) {
            trend_funStyle_graph.drawDashedLine(obj.x, obj.y + obj.height
                - count, obj.x + count, obj.y + obj.height, 0, 1,
                obj.fcolor);
            count += temp;
        }
        ;
        ...
        while (obj.x + flag + count < obj.x + obj.width) {
            trend_funStyle_graph.drawDashedLine(obj.x + count, obj.y, obj.x
                + count + flag, obj.y + obj.height, 0, 1, obj.fcolor);
            count += temp;
        }
        ;
        count = temp;
        while (obj.y + obj.height - count > obj.y) {
            trend_funStyle_graph.drawDashedLine(obj.x + obj.width - count,
                obj.y, obj.x + obj.width, obj.y + count, 0, 1, obj.fcolor);
            count += temp;
        }
        ;
    },
    ...
    drawBorderRect: function (obj) {
        trend_canvas_graph.fillRect(obj.x, obj.y, obj.width, obj.height);
        trend_funStyle_graph.drawDashedLine(obj.x, obj.y, obj.x, obj.y
            + obj.height, obj.lineStyle, obj.lineWidth, obj.color);
        trend_funStyle_graph.drawDashedLine(obj.x + obj.width, obj.y, obj.x
            + obj.width, obj.y + obj.height, obj.lineStyle, obj.lineWidth,
            obj.color);
        trend_funStyle_graph.drawDashedLine(obj.x, obj.y, obj.x + obj.width,
            obj.y, obj.lineStyle, obj.lineWidth, obj.color);
        trend_funStyle_graph.drawDashedLine(obj.x, obj.y + obj.height, obj.x
            + obj.width, obj.y + obj.height, obj.lineStyle, obj.lineWidth,
            obj.color);
    },
    /**
    ...
    drawDashedLine4BGLateral: function (temp, obj) {
        ...
        while ((obj.y + count) < obj.height + obj.y) {
            ...
            ;
            trend_funStyle_graph.drawDashedLine(obj.x + temp, yy, obj.x
                + obj.width, yy, 1, 1, obj.fcolor);
            count += temp;
        }
        ;
    },
    ...
    drawDashedLine4BGLongitudinal: function (temp, obj) {
        ...
        while ((obj.x + count) < obj.width + obj.x) {
            trend_funStyle_graph.drawDashedLine(obj.x + count, obj.y, obj.x
                + count, obj.y + obj.height, 1, 1, obj.fcolor);
            count += temp;
            var xx = obj.x + count;
            ...
            ;
            trend_funStyle_graph.drawDashedLine(xx, obj.y + temp, xx, obj.y
    ...
    drawWavesLine: function (obj) {
        ...
        while (tempY < obj.height - temp) {
            while (tempX < obj.width) {
                trend_funStyle_graph.drawDashedLine(obj.x + tempX, obj.y
                    + tempY, obj.x + tempX + temp, obj.y + tempY + temp, 0,
                    1, obj.fcolor);
                trend_funStyle_graph.drawDashedLine(obj.x + tempX + temp, obj.y
    ...
    drawBrickBG: function (obj) {
        var temp = 6, tempX = 0, tempY = 0;
        trend_funStyle_graph.drawLine4Lateral(temp, obj);
        while (tempY < obj.height - temp) {
            while (tempX + temp < obj.width) {
                trend_funStyle_graph.drawDashedLine(obj.x + tempX, obj.y
                    + tempY, obj.x + tempX, obj.y + tempY + temp, 0, 1,
                    obj.fcolor);
                trend_funStyle_graph.drawDashedLine(obj.x + tempX + temp, obj.y
    ...
    drawDashedLine: function (x, y, x2, y2, type, lw, color) {
        ...
        trend_canvas_graph.lineWidth = lw;
        trend_canvas_graph.lineCap = flag;
        trend_canvas_graph.beginPath();
        trend_canvas_graph.strokeStyle = color;
        trend_canvas_graph.dashedLine(x, y, x2, y2, dashGapArray);
        trend_canvas_graph.stroke();
    }
};
